...
function tankerStatusMessage(tanker, PlayerUnit, PlayerGroup)
    ...
    local radioFreq = 0.0
    local tacanChan = 0
    for index, value in ipairs(tankersArray) do
        if( value ~= nil) then
            if( value.tanker ~= nil ) then
                ...
                if (string.find(groupName, value.tanker.GroupName, 1, true) ~= nil) then
                    ...
                    radioFreq = value.customconfig.freq
                    tacanChan = value.customconfig.tacan.channel
                    debug_msg(string.format("%s found in %s, time in the air : %i sec, time left %i sec, TACAN : %i",
                        value.tanker.GroupName, groupName, timeInTheAir, timeLeftInTheAir, tacanChan))
                else
    ...
    for index, value in ipairs(tankersOnDemandArray) do
        env.info(tankersOnDemandArray[index])
        if( value ~= nil) then
            ...
            if (string.find(groupName, value:GetName(), 1, true) ~= nil) then
                ...
                tacanChan = value.customconfig.tacan.channel
                debug_msg(string.format("%s found in %s, time in the air : %i sec, time left %i sec, TACAN : %i",
                value:GetName(), groupName, timeInTheAir, timeLeftInTheAir, tacanChan))
            else
    ...
    local message = string.format("%s %s [%s]\nFuel State %s (%.2f)\n%s\n%s\nFreq: %.3f\nTACAN: %i\n%s", tanker:GetName(),
        ...
        radioFreq, tacanChan, timeLeftString)
